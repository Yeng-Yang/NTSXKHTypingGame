...
loginBtn.addEventListener('click', async () => {
    ...
    const { data: activeSession, error: sessionError } = await supabase_client
        .from('quiz_sessions')
        .select('id, status') // ເພີ່ມ RESULTS_AVAILABLE ເຂົ້າໃນການກວດສອບ
        .in('status', ['OPEN', 'IN_PROGRESS', 'RESULTS_AVAILABLE'])
    ...
    if (activeSession.status === 'IN_PROGRESS' || activeSession.status === 'RESULTS_AVAILABLE') {
    ...
    } else if (activeSession.status === 'RESULTS_AVAILABLE') {
        // ຖ້າຫ້ອງຢູ່ໃນສະຖານະສະແດງຜົນ, ໃຫ້ໄປທີ່ໜ້າຜົນຄະແນນ
        window.location.href = 'final_results.html';
    }
});
